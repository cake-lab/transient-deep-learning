...
parameter server was revoked. % note that, PS failed only lead to session failure, and once detected, then we have to restart from the last checkpoint model
...
\caption{\textbf{Comparison of distributed training performance using on-demand
  ...
  between on-demand and transient \texttt{K80} servers. However,
...
When swapping out two (or three) \texttt{K80} machines for more powerful GPU
